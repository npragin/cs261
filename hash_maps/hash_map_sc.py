...
# Assignment: HashMap
# Due Date: 08/13/2024
# Description: HashMap implementation using chaining
...
class HashMap:
    ...
    def put(self, key: str, value: object) -> None:
        ...
        """
        if self.table_load() >= 1:  # Handle resize of map
            self.resize_table(self._next_prime(self._capacity * 2))

        newHash = self._hash_function(key) % self._capacity  # Hash the key
        bucket = self._buckets.get_at_index(newHash)  # Get the bucket using hash

        if bucket.contains(key):  # If key already exists, remove it
            bucket.remove(key)
            self._size -= 1

        bucket.insert(key, value)  # Add new key/value pair
    ...
    def resize_table(self, new_capacity: int) -> None:
        ...
        newMap = DynamicArray()  # Instantiate new map
    ...
    def empty_buckets(self) -> int:
        ...
        for i in range(self._capacity):
            count += not self._buckets.get_at_index(i).length()  # Adds 1 if length is 0
        return count
    ...
    def get(self, key: str):
        ...
        bucket = self._buckets.get_at_index(hashedKey)
        for node in bucket:
            if node.key == key:
                return node.value
        return None
...
def find_mode(da: DynamicArray) -> tuple[DynamicArray, int]:
    """
    Given a dynamic array, returns the mode(s) and highest frequency as a tuple
    """
